...
class Planogramas_modelMike extends CI_Model
{
	...
	function traerInfoPlano($planoId){
		...
		$q = $this->db->query("SELECT
									...
									c.EstatusContrato as 'EstatusContrato',
	...
	function cargarGrupos(){
		$data = array();
		$q = $this->db->query("SELECT * FROM tempora_grupos_locales");
		if($q->num_rows() > 0) {
			foreach($q->result() as $row){
				$data[] = $row;
			}
	...
	function localesGrupos($grupo){
		$data = array();
		$q = $this->db->query("SELECT id, plazaId, grupoId FROM vector where grupoId='$grupo'");
		if($q->num_rows() > 0) {
			foreach($q->result() as $row){
				$data[] = $row;
			}
