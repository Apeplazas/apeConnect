			...
			<? foreach($planos as $row):?>
				<tr class="plaza">
                    ...
                    <th id="asigPi">
						...
						<ul>
						<? $list = $this->planogramas_modelMike->cargarPisos($row->plaza);?>
						<? foreach($list as $l):?>
                        
                        <? if($l->estado=="VENCIDO"): 
                        $var='planogramasMike/verplano/';
                        endif ?>
                        <? if($l->estado=="VIGENTE"):
						$var='planogramasMike/infoplano/'; 
						endif ?>
                       
						<li class="<?=$l->estado?>" ><a  href="<?=base_url()?><?= $var ?><?= $l->id;?>" title="Planta Baja"><?= $l->piso;?></a></li>
